...
function [p,dp,q,r,alp,dom,x,wint,wipr]=BTT_simpleexp(n)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
p=  C*expm(-X/d);% coefficient of -u''; for example p=X;
dp= -C/d*expm(-X/d);% coefficient of -u' (in fact p'(x)); for example dp=1;
q= 0   ;% coefficient of u; for example q=0;
r= eye(n);% inner product weight (matrix); for example r=2*X*(eye(n)-X^2);
wipr= ones(size(x)) ;% inner product weight (vector); for example wipr=2*x.*(1-x.^2);
